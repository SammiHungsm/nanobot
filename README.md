...
| **Neo4j Service** | v1.0 | 圖譜查詢服務（持股關係、公司網絡）|
...
│   └── static/
│
├── vanna/                      # ?? Vanna AI (Text-to-SQL) - 獨立微服務
│   └── vanna_backend/            # Vanna 實際代碼
│       ├── app_alicloud_mysql.py # Flask API 服務器 (~391行)
│       ├── training_data.py     # 訓練數據（DDL/SQL/Docs）
│       ├── utility.py           # 工具函數
...
└─────────────────────────────────────────────────────────────┘
    ...
    ▼
┌─────────────────────────────────────────────────────────────┐
│ Stage 5: Neo4j 同步 (Knowledge Graph)\n│ ├─ 實體關係同步到 Neo4j\n│ └─ 圖譜索引建立\n└─────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────┐
...
| **table** | 混亂則 Vision | ?? 條件性 | `raw_artifacts`, `document_tables` |
...
| `vanna` | 8082 | Vanna AI (Alibaba Cloud + MySQL) |
